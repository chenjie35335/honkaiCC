...
void Visit(const RawStore &data, const RawValueP &value, int id)
{ // store这个地方的加锁问题一直很大
    // cout << endl;
...
void Visit(const RawCall &data, const RawValueP &value, int id)
{
    ...
    for (int i = 0; i < 7; i++) {
            hardware.LoadRegister(RegisterManager::callerSave[i]);
    }
...
void Visit(const RawBasicBlockP &bb, int id)
{
    ...
    instAlloc.tempBlock = (RawBasicBlock *)bb;
    auto &insts = bb->inst;
    for (auto inst : insts)
        Visit(inst, id);
}
...
void Visit(const RawFunctionP &func, int id)
{
    ...
    instAlloc.tempFunction = (RawFunction *) func;
    instAlloc.tempBlock = (RawBasicBlock *)*bbs.begin();
    int bbsLen = bbs.size();
    ...
    if (bbsLen != 0)
    {
        ...
        else
        {
            cout << "li t0, " << -StackSize << endl;
            cout << "add sp, sp, t0" << endl;
            auto liT0 = AsmInst::CreateLi(T0,-StackSize);
            auto addSp = AsmInst::CreateAdd(SP,SP,T0);
...
void Visits(const RawStore &data, const RawValueP &value, list<RawValue *>::const_iterator it, RawBasicBlockP bb)
{ // store
    // cout << endl;
    const auto &src = data.value;
    const auto &dest = data.dest;
    ...
    else if (src->value.tag == RVT_AGGREGATE && dest->ty->tag == RTT_ARRAY)
    { // 这里貌似还是没有什么好办法
        // hardware.LeaseLockRegister(src);
